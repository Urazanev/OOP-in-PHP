...
class MoonPhase {
	...
	function __construct( $pdate = null ) {
		...
		$this->sunangdia = $SunAng;										// Sun's angular diameter (degrees)
                $this->sunzodiac =  $this->zodiac_sign($Lambdasun);
                $this->moonzodiac = $this->zodiac_sign($Lambdamoon);
	}
	...
	*/
	...
	private function get_phase( $n ) {
        ...
        private function zodiac_sign($i) {
            switch ($i) {
            case (($i>0) and ($i<=30)):
                $this->zodiac='Beran';
            return $this->zodiac;
        ...
        case (($i>90) and ($i<=120)):
            ...
            return $this->zodiac;
            break;
        case (($i>120) and ($i<=150)):
            $this->zodiac='Lev';
            return $this->zodiac;
            break; 
        case (($i>150) and ($i<=180)):
            $this->zodiac='Panna';
            return $this->zodiac;
            break;
        case (($i>180) and ($i<=210)):
            $this->zodiac='Váhy';
            return $this->zodiac;
            break; 
        case (($i>210) and ($i<=240)):
            $this->zodiac='Štír';
            return $this->zodiac;
            break;
        case (($i>240) and ($i<=270)):
            $this->zodiac='Střelec';
            return $this->zodiac;
            break; 
        case (($i>270) and ($i<=300)):
            $this->zodiac='Kozoroh';
            return $this->zodiac;
            break;
        case (($i>300) and ($i<=330)):
            $this->zodiac='Vodnář';
            return $this->zodiac;
            break;
        case (($i>330) and ($i<=360)):
            $this->zodiac='Ryby';
            return $this->zodiac;
            break;
            }
	...
	function phase_name() {
		$names1 = array( 'New Moon', 'Waxing Crescent', 'First Quarter', 'Waxing Gibbous', 'Full Moon', 'Waning Gibbous', 'Third Quarter', 'Waning Crescent', 'New Moon' );
		$names = array( 'Nov', 'Dorůstající srpek', 'První čtvrt', 'Dorůstající měsíc', 'Úplněk', 'Couvající měsíc', 'Poslední čtvrť', 'Ubývající srpek', 'Nov' );
